...
<html>
	...
	<body>
		...
		<nav>
			...
			<div class="container">
				<form style="text-align:center" class="main-cont addemp-form" method="post" action="editemp_2.php">
					<h2>Edit Employee Info</h2><br><br>
					<input name="EmpID" type="text" placeholder="Employee ID" required /><br><br>
					<button style="margin: 0 auto;width: 15%;" type="submit" class="btn btn-danger">Edit</button>
   					<div id="error-cont">
					<?php
					if($_GET["q"]==1)
{
echo '<script>						$("#error-cont").html("<h4>No such employee exists</h4>");
					...
					if($_GET["q"]==2)
{
echo '<script>						$("#error-cont").html("<h4>Sorry, Manager cannot be edited.</h4>");
						$("#error-cont").css({"padding": "10px"})
</script>';
}
					if($_GET["q"]==2.5)
...
echo '<script>						$("#error-cont").html("<h4>No such department exists</h4>");
					...
					if($_GET["q"]==3)
{
echo '<script>						$("#error-cont").html("<h4>Employee successfully edited.</h4>");
						$("#error-cont").css({"padding": "10px"})
</script>';
}
					?>
