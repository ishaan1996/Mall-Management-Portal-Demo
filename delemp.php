...
if(isset($_POST["EmpID"]))
{
	...
	if(mysql_num_rows($rs)==0)
		header('location:http://localhost/DBProject/delemp.php?q=1');
	else
	{
		$q="select * from User where ID=".$_POST["EmpID"];
		$rs=mysql_query($q,$con);
		echo $q;
		$row=mysql_fetch_array($rs);
		if($row["Type"]==1 or $row["Type"]==3)
		{
			header('location:http://localhost/DBProject/delemp.php?q=2');
			exit();
		}
		$q="delete from Employee where EmpId=".$_POST["EmpID"];
		$rs=mysql_query($q,$con);
		echo $q;
		$q="delete from User where ID=".$_POST["EmpID"];
		$rs=mysql_query($q,$con);
		echo $q;
		header('location:http://localhost/DBProject/delemp.php?q=3');
	}
}
?>
<html>
	...
	<body>
		...
		<nav>
			...
			<div class="container">
				<form style="text-align:center" class="main-cont addemp-form" method="post" action="delemp.php">
					<h2>Delete Employee</h2><br><br>
					<input name="EmpID" type="text" placeholder="Employee ID" /><br><br>
					<button style="margin: 0 auto;width: 15%;" type="submit" class="btn btn-danger">Delete</button>
   					<div id="error-cont">
					<?php
					if($_GET["q"]==1)
{
echo '<script>						$("#error-cont").html("<h4>No such employee exists</h4>");
						$("#error-cont").css({"padding": "10px"})
</script>';
}
					if($_GET["q"]==2)
{
echo '<script>						$("#error-cont").html("</h4>Sorry, Manager cannot be deleted.</h4>");
						$("#error-cont").css({"padding": "10px"})
</script>';
}
					if($_GET["q"]==3)
{
echo '<script>						$("#error-cont").html("<h4>Employee successfully deleted.</h4>");
						$("#error-cont").css({"padding": "10px"})
</script>';
}
					?>
