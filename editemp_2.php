...
if(mysql_num_rows($rs)==0)
	header('location:http://localhost/DBProject/editemp_1.php?q=1');
else
{
	$q1="select * from User where ID=".$_POST["EmpID"];
	$rs1=mysql_query($q1,$con);
//	echo $q1;
	$row1=mysql_fetch_array($rs1);
	if($row1["Type"]==1 or $row1["Type"]==3)
	{
		if($_POST["EmpID"]!=$_SESSION["Owner"])
		{
			header('location:http://localhost/DBProject/editemp_1.php?q=2');
			exit();
		}
	}
}
...
<html>
	...
	<body>
		...
		<nav>
			...
			<div class="container">
				<form class="main-cont addemp-form" method=post action="editemp_2_process.php">
					<span style="display:inline" class="glyphicon glyphicon-edit"></span>
					<h2>Edit Employee Info</h2>
					<table>
						<tr class="one-half">
							<td colspan=2 ><input name="EmpId" type="hidden" value="<?php echo $_POST["EmpID"]; ?>" required /></td>
						</tr>
						<tr class="one-half">
							<td>Name</td>
