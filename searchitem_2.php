...
{
	header('location:http://localhost/DBProject/searchitem_1.php?q=1');
	exit();
}
else
{
	$q1="select * from Warehouse where WId=".$row['WId'];
	$rs1=mysql_query($q1,$con);
	$row1=mysql_fetch_array($rs1);
	$q2="select * from Category where CategoryId=".$row['CategoryId'];
	$rs2=mysql_query($q2,$con);
			...
			<div class="container">
				<form class="main-cont addemp-form">
					...
					<table>
						<tr class="one-half">
							<td>Product Name</td>
							<td><input type="text" value="<?php echo $row["ProdName"]; ?>" disabled /></td>
						</tr>
						<tr class="one-half">
							<td>Price</td>
							<td><input type="text" value="<?php echo $row["Price"]; ?>" disabled /></td>
						</tr>
						<tr class="one-half">
							<td>Quantity In Stock</td>
							<td><input type="text" value="<?php echo $row["Qty"]; ?>" disabled /></td>
						</tr>
						<tr class="one-half">
							<td>Minimum Quantity</td>
							<td><input type="text" value="<?php echo $row["MinQty"]; ?>" disabled /></td>
						</tr>
						<tr class="one-half">
							<td>Warehouse Name</td>
							<td><input type="text" value="<?php echo $row1["WName"]; ?>" disabled /></td>
						</tr>
						<tr class="one-half">
							<td>Category</td>
							<td><input type="text" value="<?php echo $row2["CategoryName"]; ?>" disabled /></td>
						</tr>
						<tr class="one-half">
							<td>Description</td>
							<td><textarea col="20" row="20" disabled ><?php echo $row["Description"]; ?></textarea></td>
						</tr>
					</table>
				</form> 
			</div>
