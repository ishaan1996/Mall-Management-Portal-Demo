...
<html>
	...
	<body>
		...
		<nav>
			...
			<div class="container">
				<form style="text-align:center" class="main-cont addemp-form" method="post" action="edititem_2.php">
					<h2>Edit Item Info</h2><br><br>
					<input name="pid" type="text" placeholder="Product ID" /><br><br>
					<button style="margin: 0 auto;width: 15%;" type="submit" class="btn btn-danger">Edit</button>
  					<div id="error-cont">
					<?php
					if($_GET["q"]==1)
{
echo '<script>						$("#error-cont").html("<h4>No such product exists</h4>");
						$("#error-cont").css({"padding": "10px"})
</script>';
}
					else if($_GET["q"]==2)
{
echo '<script>						$("#error-cont").html("<h4>Product successfully updated.</h4>");
						$("#error-cont").css({"padding": "10px"})
</script>';
}
					?>
